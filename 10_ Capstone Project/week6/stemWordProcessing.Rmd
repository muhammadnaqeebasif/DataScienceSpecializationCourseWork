...
if(!file.exists("x_y_data.csv")){
  if (!file.exists("encoded_data.RData")){
    ...
    # removing the line with length = 1
    l <- map_dbl(encoded_data,length)
    encoded_data <- encoded_data[l>1]
...
if(!file.exists("x_y_data.csv")){
  if(!file.exists("x_y_data1.RData")){
    x_y_data_1 <- future_map_dfr(encoded_data[1:(length(encoded_data)/2)],
                                 map_xy_dfr,.progress=TRUE)
